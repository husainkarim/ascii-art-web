...
func HomeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		err = helper.Createfile("txt", data.AsciiArtDraw) // create the file with extension .txt
		if err != nil {
			http.Redirect(w, r, "/400", http.StatusSeeOther)
			return
		}
		// set file name and the download button for file with extension .txt
		txtfile := "file.txt"
		...
		if filetype != "" { // if the user requested a extra file type
			err = helper.Createfile(filetype, data.AsciiArtDraw) // create a new file with the extension givin
			if err != nil {
				http.Redirect(w, r, "/400", http.StatusSeeOther)
				return
			}
			// set the file name and the download button
